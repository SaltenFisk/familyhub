...
import EmailModal, { type DemoData } from '../components/EmailModal'
...
const DEMO_CATEGORY_COLOUR: Record<string, 'blue' | 'purple' | 'green' | 'amber'> = {
  Jill: 'green', Mark: 'amber', Hope: 'blue', Charles: 'purple',
}

const DEMO_ALL_WHO = ['Jill', 'Mark', 'Hope', 'Charles'] as const

// Demo attachment: email_id 2 has a scout camp kit list
const DEMO_ATTACHMENTS: Record<number, DemoData['attachments']> = {
  2: [{ id: 1, filename: 'summer-camp-kit-list.pdf', label: 'summer-camp-kit-list.pdf' }],
}
...
export default function DemoDashboard() {
  ...
  function handleArchiveEmailClick(emailId: number) {
    const task = tasks.find(t => t.email_id === emailId)
    if (task) setSelectedTask(task)
  }

  // Build DemoData for EmailModal
  const demoModalData: DemoData = {
    bodyText: selectedTask ? DEMO_EMAIL_BODIES[selectedTask.email_id]?.body_text : undefined,
    familyMembers: DEMO_FAMILY,
    attachments: selectedTask ? DEMO_ATTACHMENTS[selectedTask.email_id] : undefined,
    categoryColour: DEMO_CATEGORY_COLOUR,
    allWho: DEMO_ALL_WHO,
    onSave: (updated) => {
      setTasks(prev => prev.map(t => t.id === updated.id ? updated : t))
    },
    onAssign: (taskId, name) => {
      setTasks(prev => prev.map(t => t.id === taskId ? { ...t, assignee_name: name || null } : t))
    },
  }
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex flex-col">
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-4 py-6 pb-24 md:pb-6 space-y-5">
        {activeTab === 'archive' ? (
          <EmailArchive onEmailClick={handleArchiveEmailClick} staticEmails={DEMO_EMAILS} />
        ) : (
          <>
            {/* Filter bar */}
