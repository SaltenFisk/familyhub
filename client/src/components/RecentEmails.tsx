  ...
  return (
    <div>
      <div className="divide-y divide-slate-100 dark:divide-slate-700/50">
        {emails.length === 0 && (
          <p className="text-sm text-slate-400 py-6 text-center">No emails yet</p>
        )}
        {emails.map(email => (
          <button
            key={email.id}
          ...
          >
            <div className="flex-1 min-w-0">
              <p className="text-sm font-medium text-slate-700 dark:text-slate-300 truncate group-hover:text-orange-500 transition-colors">
